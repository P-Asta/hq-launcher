...
import { FolderOpen, Keyboard, Pencil, RefreshCw, X } from "lucide-react";
import { Button } from "./components/ui/button";
import { Slider } from "./components/ui/slider";
import { cn } from "./lib/cn";
import { KeyCaptureButton, ModuleSettings, ResetButton } from "./components/overlay/ModuleSettings";
import { OverlayModuleView } from "./components/overlay/OverlayModuleView";
import { OVERLAY_ENEMY_CATALOG, normalizedEnemyCounts } from "./overlay/enemies";
import { FALLBACK_MODULES } from "./overlay/fallbackModules";
import { HQ_LEADERBOARD_BOARD_TYPES, HQ_LEADERBOARD_COLLECTIONS, HQ_LEADERBOARD_RUN_FIELDS, calculateLeaderboardCheck, createLcStatsView, fetchHqRuns, getLeaderboardInput } from "./overlay/hqLeaderboard";
import { canonicalShortcut, collectModuleInputShortcuts, createInputSnapshot, eventShortcutFromKeyboardEvent, normalizeInputKeyName, shortcutParts } from "./overlay/input";
import { isInteractiveDragTarget, normalizePosition, overlayRenderEntries, snapOverlayPosition } from "./overlay/layout";
import { DEFAULT_CONFIG, createModuleApi, evaluateModule, extractStreamOverlaysPayload, normalizeConfig, overlayErrorMessage, overlayErrorStack, resetValueForSetting } from "./overlay/moduleRuntime";
import { normalizedScrapSummary } from "./overlay/scrap";
import { escapeHtml, formatSeconds, intish, number, stripLcQuote, valueAt, valueAtAny } from "./overlay/values";
...
export default function GameOverlay({ captureOnly = false }) {
  ...
  useEffect(() => {
    const makeKeyHandler = (type) => (event) => {
      ...
      if (type === "keydown") down.add(shortcut);
      else down.delete(shortcut);
      pushInputEvent({
    ...
    const handleKeyDown = makeKeyHandler("keydown");
    const handleKeyUp = makeKeyHandler("keyup");
