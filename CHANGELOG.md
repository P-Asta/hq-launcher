...
- LCStatsTracker AutoSheet tracking now stops automatically when the launched game exits or the final running game is stopped.
...
- Added **Custom Layout** collected bee value support, with optional notes listing collected hive values.
- Added a **Custom Layout Enemy** group with code-name based enemy columns, shared `Write false` and `Write 0` options, and death enemy notes that append pre-death spawn information after death reason notes.
...
- Improved LCStatsTracker AutoSheet stat parsing to support the newer nested `PerformanceInfo`, `QuotaInfo`, and `EventInfo` payload keys while keeping fallback support for older flat keys.
- Updated AutoSheet layouts to use the newer available-value fields consistently and to treat **Galetry** like other economy moons where applicable.
...
- Improved the Custom Layout settings expander so large sections are no longer clipped.
- Moved the remote manifest documentation under `docs/`.
...
- **Patch Security Vulnerability In Older Versions [#9](https://github.com/P-Asta/hq-launcher/pull/9)
