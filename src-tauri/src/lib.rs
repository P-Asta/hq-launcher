...
use windows_sys::Win32::Foundation::{
    CloseHandle, GetLastError, HWND, INVALID_HANDLE_VALUE, LPARAM, RECT, WAIT_OBJECT_0,
    WAIT_TIMEOUT,
};
#[cfg(target_os = "windows")]
...
#[cfg(target_os = "windows")]
const GAME_OVERLAY_INPUT_POLL_INTERVAL_MS: u64 = 8;
#[cfg(target_os = "windows")]
const GAME_OVERLAY_INPUT_IDLE_INTERVAL_MS: u64 = 100;
...
#[cfg(target_os = "windows")]
const VK_LEFT_KEY: i32 = 0x25;
#[cfg(target_os = "windows")]
const VK_UP_KEY: i32 = 0x26;
#[cfg(target_os = "windows")]
const VK_RIGHT_KEY: i32 = 0x27;
#[cfg(target_os = "windows")]
const VK_DOWN_KEY: i32 = 0x28;
#[cfg(target_os = "windows")]
...
fn replace_cfg_bool_assignment(text: &str, key: &str, desired_value: bool) -> (String, bool) {
    let mut changed = false;
    let mut out = String::with_capacity(text.len());
    let desired = if desired_value { "true" } else { "false" };
    ...
    for seg in text.split_inclusive(['\n', '\r']) {
        ...
        if let Some(eq_idx) = trimmed.find('=') {
            ...
            if left.trim().eq_ignore_ascii_case(key) {
                ...
                if !right.trim().eq_ignore_ascii_case(desired) {
                    out.push_str(indent);
...
fn ensure_unity_explorer_eventsystem_override_in_dir(
    config_dir: &std::path::Path,
) -> Result<bool, String> {
    let cfg_path = config_dir.join("com.sinai.unityexplorer.cfg");
    if !cfg_path.exists() {
        return Ok(false);
    }
    ...
    let text = String::from_utf8_lossy(&bytes);
    let (updated, changed) =
        replace_cfg_bool_assignment(&text, "Disable EventSystem override", true);
    if changed {
...
fn migrate_disablemod_v6_unity_explorer_cfg(app: &tauri::AppHandle) -> Result<(), String> {
    let mut config_dirs = vec![shared_config_dir(app)?];

    for version in list_installed_versions(app.clone())? {
        config_dirs.push(version_config_dir(app, version)?);
    }

    let mut seen = HashSet::new();
    for config_dir in config_dirs {
        let key = std::fs::canonicalize(&config_dir)
        ...
        if let Err(e) = ensure_unity_explorer_eventsystem_override_in_dir(&config_dir) {
            log::warn!(
                "Failed to migrate UnityExplorer config in {}: {e}",
...
impl Default for GameOverlayGeneralConfig {
    fn default() -> Self {
        Self {
            ...
            monitor_interval_ms: GAME_OVERLAY_MONITOR_INTERVAL_DEFAULT_MS,
        }
    }
}
...
mod game_overlay_backend_tests {
    ...
    fn polled_overlay_shortcut_emits_only_state_transitions() {
        let down = Mutex::new(HashSet::new());
        assert!(matches!(
            update_game_overlay_shortcut_state(&down, "Insert", true),
            Some(ShortcutState::Pressed)
        ));
        assert!(update_game_overlay_shortcut_state(&down, "Insert", true).is_none());
        assert!(matches!(
            update_game_overlay_shortcut_state(&down, "Insert", false),
            Some(ShortcutState::Released)
        ));
        assert!(update_game_overlay_shortcut_state(&down, "Insert", false).is_none());
    }
...
struct GameOverlayState {
    ...
    input_monitor_running: AtomicBool,
...
impl Default for GameOverlayState {
    fn default() -> Self {
        Self {
            ...
            input_monitor_running: AtomicBool::new(false),
...
fn read_disablemod(app: &tauri::AppHandle) -> Result<DisableModFile, String> {
    ...
    let original_version = f.version;
    ...
    if original_version <= 5 {
        if let Err(e) = migrate_disablemod_v6_unity_explorer_cfg(app) {
            log::warn!("Failed to migrate UnityExplorer config for disablemod v6: {e}");
        }
        f.version = DISABLEMOD_FILE_VERSION;
        let _ = write_disablemod(app, &f);
    }
...
#[cfg(target_os = "windows")]
fn overlay_shortcut_is_pressed(shortcut: &str, current: OverlayShortcutModifierState) -> bool {
    overlay_shortcut_specs_for_key(shortcut)
        .into_iter()
        .any(|spec| {
            spec.modifiers == current && unsafe { GetAsyncKeyState(spec.vk_code as i32) < 0 }
        })
}
...
fn hide_game_overlay_window(app: &tauri::AppHandle, close_controls: bool) {
    ...
    if let Some(window) = app.get_webview_window(GAME_OVERLAY_WINDOW_LABEL) {
        ...
        let _ = window.hide();
        set_game_overlay_frontend_active(app, false);
    ...
    tauri::async_runtime::spawn(async move {
        loop {
            ...
            match discord_presence::receive_stream_overlays_data() {
                Some(payload) => {
                    let _ = app_handle.emit("overlay://stream-overlays-updated", &payload);
                }
                None => {}
            }
...
fn set_game_overlay_frontend_active(app: &tauri::AppHandle, active: bool) {
...
fn deactivate_game_overlay_frontend_after_focus_loss(
    app: &tauri::AppHandle,
    overlay_state: &GameOverlayState,
    close_controls: bool,
) {
    if close_controls || overlay_state.controls_open.load(Ordering::Relaxed) {
        overlay_state.controls_open.store(false, Ordering::Relaxed);
        if let Some(window) = app.get_webview_window(GAME_OVERLAY_WINDOW_LABEL) {
            let _ = apply_game_overlay_interaction(&window, false);
        }
        let _ = app.emit_to(
...
#[cfg(target_os = "windows")]
fn game_overlay_should_ignore_window_management_shortcut() -> bool {
    fn key_down(vkey: i32) -> bool {
        unsafe { GetAsyncKeyState(vkey) < 0 }
    }
...
#[cfg(target_os = "windows")]
fn clear_game_overlay_input_down_state(app: &tauri::AppHandle) {
    let state = app.state::<GameOverlayState>();
...
#[cfg(target_os = "windows")]
fn update_game_overlay_shortcut_state(
    ...
    if pressed {
        down.insert(shortcut.to_string())
            .then_some(ShortcutState::Pressed)
    } else {
        down.remove(shortcut).then_some(ShortcutState::Released)
    }
}

#[cfg(target_os = "windows")]
fn poll_game_overlay_input_shortcuts(app: &tauri::AppHandle) {
    if !should_emit_game_overlay_input(app) {
        clear_game_overlay_input_down_state(app);
        return;
    }

    let state = app.state::<GameOverlayState>();
    let modifiers = current_overlay_shortcut_modifier_state();
    ...
    if let Some(shortcut) = overlay_shortcut {
        let pressed = overlay_shortcut_is_pressed(&shortcut, modifiers);
        if let Some(shortcut_state) =
            update_game_overlay_shortcut_state(&state.overlay_shortcut_down, &shortcut, pressed)
        {
    ...
    for shortcut in watched {
        let pressed = overlay_shortcut_is_pressed(&shortcut, modifiers);
        if let Some(shortcut_state) = update_game_overlay_shortcut_state(
            &state.overlay_input_down_shortcuts,
            &shortcut,
            pressed,
        ) {
...
#[cfg(target_os = "windows")]
fn game_overlay_input_monitor_needed(app: &tauri::AppHandle) -> bool {
    let overlay_state = app.state::<GameOverlayState>();
    if !overlay_state.overlay_enabled.load(Ordering::Relaxed)
        && !overlay_state.preview_mode.load(Ordering::Relaxed)
        && !overlay_state.controls_open.load(Ordering::Relaxed)
...
#[cfg(target_os = "windows")]
fn start_game_overlay_input_monitor(app: &tauri::AppHandle) {
    let state = app.state::<GameOverlayState>();
    if state.input_monitor_running.swap(true, Ordering::Relaxed) {
        return;
    }
    ...
    std::thread::spawn(move || {
        ...
        loop {
            let active = game_overlay_input_monitor_needed(&app_handle);
            if active {
                was_active = true;
                poll_game_overlay_input_shortcuts(&app_handle);
                std::thread::sleep(std::time::Duration::from_millis(
                    GAME_OVERLAY_INPUT_POLL_INTERVAL_MS,
                ));
                continue;
            }

            if was_active {
                clear_game_overlay_input_down_state(&app_handle);
                was_active = false;
            }
            std::thread::sleep(std::time::Duration::from_millis(
                GAME_OVERLAY_INPUT_IDLE_INTERVAL_MS,
            ));
        }
    });
    set_game_overlay_debug(app, "non-blocking overlay input monitor started");
}
...
fn emit_game_overlay_input_shortcut(
    ...
    let _ = app.emit("overlay://input-shortcut", payload);
}
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let Some((target_pid, _)) = overlay_target_window(&active_pids) else {
            ...
            set_game_overlay_error(
                app,
                format!(
                    "{shortcut_label} pressed but no Lethal Company window was found ({foreground})"
                ),
            );
            return;
        };
...
fn register_game_overlay_shortcut(app: &tauri::AppHandle) {
    ...
    {
        #[cfg(target_os = "windows")]
        start_game_overlay_input_monitor(app);
        return;
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        *registered = Some(shortcut_label.clone());
        ...
        set_game_overlay_debug(
            app,
            format!("registered overlay shortcut monitor {shortcut_label}"),
        );
        return;
    }
...
fn is_unity_explorer_mod_name(name: &str) -> bool {
    let normalized: String = name
        .chars()
        .filter(|c| c.is_ascii_alphanumeric())
        .flat_map(|c| c.to_lowercase())
        .collect();
    normalized.contains("unityexplorer")
}

fn config_path_matches_mod(path: &str, dev: &str, name: &str) -> bool {
    let lp = path.to_lowercase();
    let d = dev.to_lowercase();
    let n = name.to_lowercase();
    if (!d.is_empty() && lp.contains(&d)) || (!n.is_empty() && lp.contains(&n)) {
        return true;
    }

    let file_name = lp.rsplit('/').next().unwrap_or(&lp);
    is_unity_explorer_mod_name(name) && file_name == "com.sinai.unityexplorer.cfg"
}
...
fn list_config_files_for_mod(
    ...
    Ok(all
        .into_iter()
        .filter(|p| config_path_matches_mod(p, &dev, &name))
        .collect())
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            #[cfg(target_os = "windows")]
            start_game_overlay_input_monitor(app.handle());
            #[cfg(target_os = "windows")]
